...
import { IpUser } from '../interface/ip-user';
...
export class MStatsComponent implements OnInit, AfterViewInit {
  ...
  // dataSource = new MatTableDataSource<IpUser>(ELEMENT_DATA);
  ...
  findAll() {
    this.mStatsSvc.findAll().subscribe((resp: any) => {
      ...
      this.respUtil.load(resp.status, () => {
        ...
        console.log('syahir response', res);

        this.dataSource = new MatTableDataSource<IpUser>(res);
        this.dataSource.paginator = this.paginator;
      })
